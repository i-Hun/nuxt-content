...
const { Markdown, YAML, CSV, XML, IPYNB } = require('../parsers')

const QueryBuilder = require('./query-builder')
const EXTENSIONS = ['.md', '.json', '.json5', '.yaml', '.yml', '.csv', '.xml', '.ipynb']
...
class Database extends Hookable {
  constructor (options) {
    ...
    this.ipynb = new IPYNB(options.ipynb)
  ...
  async parseFile (path) {
    ...
    const parser = ({
      ...
      '.ipynb': data => this.ipynb.toJSON(data),
      ...this.extendParser
    })[extension]
